...
(
                                      1
) DEFAULT 0 COMMENT 'True, if this user is a bot',
    `is_registered` tinyint
(
    1
) DEFAULT 0 COMMENT 'True, if this user registered',
    `is_full_registered` tinyint
(
    1
) DEFAULT 0 COMMENT 'True, if this user full registered on site',
    `is_banned` tinyint
(
    1
) DEFAULT 0 COMMENT 'True, if this user banned',
    `first_name` CHAR
(
    255
) NOT NULL DEFAULT '' COMMENT 'User''s or bot''s first name',
    `last_name` CHAR
(
    255
) DEFAULT NULL COMMENT 'User''s or bot''s last name',
    `username` CHAR
(
    191
) DEFAULT NULL COMMENT 'User''s or bot''s username',
    `language_code` CHAR
(
    10
) DEFAULT NULL COMMENT 'IETF language tag of the user''s language',
    `is_premium` tinyint
(
    1
) DEFAULT 0 COMMENT 'True, if this user is a Telegram Premium user',
    `added_to_attachment_menu` tinyint
(
    1
) DEFAULT 0 COMMENT 'True, if this user added the bot to the attachment menu',
