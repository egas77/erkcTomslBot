...
class Conversation
{
     ...
     * @var mixed
     */
    protected $protected_receipt_details;
    ...
    protected function clear(): bool
    {
        ...
        $this->notes = null;
        $this->protected_receipt_details = null;
    ...
    protected function load(): bool
    {
        ...
        if (isset($conversation[0])) {
            ...
            //Load the conversation keyboards
            $this->protected_keyboards = json_decode($this->conversation['keyboards'], true);
            $this->keyboards = $this->protected_keyboards;

            $this->protected_attempts = $this->conversation['attempts'];
            $this->attempts = $this->protected_attempts;

            $this->protected_receipt_details = json_decode($this->conversation['receipt_details'], true);
            $this->receipt_details = $this->protected_receipt_details;
        }
    ...
    public function update(): bool
    {
        if ($this->exists()) {
            $fields = [
                'notes' => json_encode($this->notes, JSON_UNESCAPED_UNICODE),
                'receipt_details' => json_encode($this->receipt_details, JSON_UNESCAPED_UNICODE),
    ...
    public function getReceipt_details(): int
    {
